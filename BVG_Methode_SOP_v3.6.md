...
VERSIE: v3.6 (concept ter review)
DATUM: 14 juni 2026
VORIGE: v3.5 (zelfde dag) <- v3.4 (12 juni 2026)
ENGINE: v1.9.0 (taak A+B geïmplementeerd & deels gevalideerd; zie status onder)

REPO (GitHub): bvgcoaching/BVG-Methode
  Bevat: engine_v1.9.0.py, bibliotheek_v1.7.json, deze SOP, BUGRAPPORT_v1.9.0.md.
...
ENGINE-IMPLEMENTATIESTATUS:
  ...
  OPEN BUG (BUGRAPPORT_v1.9.0.md): bij krappe profielen (3d/75min) krijgt billen
    wel volume maar de SELECTIE-MOTOR plaatst geen bil-oefeningen. Volume-laag OK,
    plaatsing-laag knelt. Te onderzoeken: harde_filters + rondes-logica (+ taak C).
  NOG NIET IN ENGINE (openstaande taken):
    - Taak C: herstel-plafond OPGEHELDERD (bestaat niet meer, niets te kalibreren).
      Resteert: bil-selectie-bug (zie boven) + optionele spillover-verrekening in
      bil-telling. VEREIST draaiende engine + validatie Owen/Romme.
...
# DE BVG METHODE — COMPLETE SOP (v3.6)
...
> - **In engine v1.9.0** (geïmplementeerd & deels gevalideerd): de vrouw-billen-regel landmark-laag (§5/§11) en de vrouw-cap-bonus +2 (§3). Geen regressie voor mannen. **Open:** bil-selectie-bug bij krappe profielen (billen krijgt volume maar geen oefeningen — zie `BUGRAPPORT_v1.9.0.md`). Het oude herstel-plafond bleek al vervangen (geen werk nodig, §3·HERSTEL).
...
> 2. **Bil-selectie-bug** (engine v1.9.0): bij krappe profielen krijgt billen volume maar plaatst de selectie-motor geen bil-oefeningen. Zie `BUGRAPPORT_v1.9.0.md`. (Het oude herstel-plafond is opgehelderd — bestaat niet meer, zie §3·HERSTEL.)
...
> ✅/⚠ **Engine-status (v1.9.0):** taak A is grotendeels geïmplementeerd. Billen krijgt voor vrouwen eigen landmarks `RP_LANDMARKS_BILLEN_VROUW = (4,8,12,16,20)` via `billen_landmark(geslacht)`, en wordt niet meer als accessoire geroteerd maar als primaire spier behandeld. Geverifieerd: geen regressie voor mannen; vrouw-bilvolume schaalt correct bij ruimere profielen (4 dagen → 13 sets). **Open punt:** bij krappe profielen (3 dagen/75 min) plaatst de selectie-motor géén bil-oefeningen ondanks toegewezen volume — aparte bug, gedocumenteerd in `BUGRAPPORT_v1.9.0.md`, te onderzoeken in de selectie-/plaatsingslaag (samen met taak C). De A3-herverdeling is bewust NIET gebouwd (zie kader hierboven). De expliciete spillover-verrekening in de selectie blijft taak C.
...
- **Taak A — vrouw-billen-regel** (§5/§11). ✅ Landmark-laag geïmplementeerd in engine v1.9.0: billen primair voor vrouwen met eigen landmarks, geen regressie voor mannen, schaalt correct bij ruimere profielen. ⚠ Open: selectie-bug bij krappe profielen (bil-volume toegewezen maar geen oefeningen geplaatst) — zie `BUGRAPPORT_v1.9.0.md`. A3-herverdeling geschrapt (besluit 14/6). A4 (Hip Adduction her-tag) → taak D.
- **Taak B — vrouw-cap +2** (§3). Wijzigingsbestand `TAAK_B_vrouw_cap_bonus.md` klaar. Eén constante.
- **Taak C — herstel-plafond OPGEHELDERD** (§3·HERSTEL): bestaat niet meer in de engine (vervangen door landmark + sessie-cap + SESSIE_SPIER_CAP=8). Niets te kalibreren. **Resteert:** de bil-selectie-bug (`BUGRAPPORT_v1.9.0.md`) + optionele spillover-verrekening in de bil-telling. ⚠ Selectie-bug vereist draaiende engine + validatie op Owen/Romme.
...
- **v3.6 (14 juni 2026):** na implementatie + validatie van taak A/B in engine v1.9.0. **Bil-voorkeur herzien:** de "proportionele herverdeling met MEV-vloer" (v3.5) is GESCHRAPT — bij het draaien van de engine bleek dat onnodig (dag-cap + spillover geven al ~10 effectieve bil-sets) én onwenselijk (quad/ham wegnemen schaadt beenbalans). Bil-voorkeur tilt billen nu naar primair-niveau via landmarks + voorkeur-lean; dag-cap bepaalt de balans. Flag `BIL_VOORKEUR_GEKNELD_DOOR_MEV_VLOER` verwijderd. Engine-status bijgewerkt: taak A landmark-laag ✅ in v1.9.0, selectie-bug bij krappe profielen ⚠ gedocumenteerd (BUGRAPPORT_v1.9.0.md).
...
- **Doc/engine-status:** taak A (vrouw-billen landmark-laag) en taak B (vrouw-cap +2) zijn geïmplementeerd in engine v1.9.0 en deels gevalideerd. Het oude herstel-plafond bleek al vervangen (geen werk nodig). Resterende divergentie: de bil-selectie-bug bij krappe profielen (engine produceert volume zonder oefeningen) — gedocumenteerd in BUGRAPPORT_v1.9.0.md, op te lossen in een vervolgsessie.
