...
VERSIE: v4.0 (concept ter review)
DATUM: 14 juni 2026
VORIGE: v3.9 (15 juni) <- v3.8 <- v3.7 <- v3.4 (12 juni)
ENGINE: v1.9.0 (taak A+B geïmplementeerd & deels gevalideerd; zie status onder)

REPO (GitHub): bvgcoaching/BVG-Methode
  Bevat: engine_v1.9.0.py (=v1.9.5), bibliotheek_v1.10.json, deze SOP, STATUSRAPPORT_v1.9.5.md.
...
# DE BVG METHODE — COMPLETE SOP (v4.0)
...
- **Core-plaatsingsvoorkeur (besluit Bas, 16 juni):** core landt bij voorkeur op **Lower / Legs / Full Body** — daar past romp-stabiliteitswerk natuurlijk bij de sessie. Op **Upper** alleen als die dag nog ruimte heeft; Upper-dagen zitten meestal al vol met druk/trek-werk. Dit is een **zachte volgordevoorkeur, geen uitsluiting** — core blijft op elk dagtype toegestaan (zie §2-dagtypes "(+ Core)"), maar wordt bij gelijke ruimte eerst op de onderlijf-/FB-dag geplaatst. Het harde weekminimum van 2 (§hieronder) en de min-2-vloer blijven leidend. *Status: handmatig toegepast per schema; engine-implementatie geparkeerd tot het patroon over meerdere klanten bevestigd is.*
...
**Blessure-matching (synoniem-bewust, v1.9.5):** blessure-invoer is vrije coach-tekst. De engine matcht via een synoniem-map met substring-zoeking op de canonieke contra-indicaties (knie · schouder · onderrug · elleboog · pols). Zo sluiten "knieblessure", "knieën", "lage rug", "rugpijn", "lage rugklachten", "polsblessure", "tenniselleboog", "schouderblessure" enz. correct de juiste oefeningen uit. Exacte-token-match (oude gedrag) liet deze varianten lekken — opgelost.

**Blessure die een hele spier uitsluit (Optie A):** sluiten alle oefeningen van een PRIMAIRE spier uit door een blessure (bv. knie → alle quads, elleboog → alle biceps), dan programmeert de engine die spier NIET en forceert nooit een gecontra-indiceerde oefening. In plaats daarvan komt er een leesbare instructie in de output-key `coach_instructies` (welke spier, welke blessure, "vul handmatig in met revalidatie-aanpak"). BEWUST GEEN globale "blessure-veilig"-vlag op oefeningen: of een oefening veilig is hangt af van de specifieke blessure (ACL ≠ patellapees ≠ meniscus) — dat blijft coach-oordeel per klant, niet een one-size-fits-all aanname in de bibliotheek.
...
- **v4.0 (15 juni 2026):** **blessure-robuustheid + Optie A.** (1) Synoniem-bewuste blessure-matching (substring): "lage rug"/"knieblessure"/"rugpijn"/"polsblessure"/"tenniselleboog" sluiten nu correct uit — exacte-token-match liet deze lekken (veiligheidsfix). (2) Nieuwe output-key `coach_instructies`: leesbare instructie wanneer een primaire spier niet veilig geprogrammeerd kan worden (blessure sluit alle oefeningen uit); engine forceert nooit een gecontra-indiceerde oefening. GEEN globale blessure-veilig-vlag (blijft coach-oordeel per klant). Engine v1.9.5. Geaudit: 512-grid schoon, 0 valse coach-instructies bij gezonde klanten, 14 synoniemen correct, geen lek bij enkele/combi/alle-5 blessures. Zie STATUSRAPPORT_v1.9.5.md.
- **v3.9 (15 juni 2026):** **taak F** — Full Body-dagen lean & professioneel. F1: mannen geen glute-fill meer (bilwerk via spillover + 45 back ext). F2: schouders max 2 sets/oefening. F3: overhead press toegestaan op FB-dagen (Upper blijft lateraal/achter); pols/schouder-blessure sluit press uit via nieuwe contra-indicatie. F4: armen (bi 2 + tri 2) op elke FB-dag. F5: max 1 roterend accessoire per FB-dag (per cyclus+dagindex), prehab-slot van FB-dag af. Bibliotheek -> v1.10 (Pols-contra op 4 overhead presses). Geaudit: 512-grid schoon, volume consistent, MEV+frequentie gehaald, 13 adversariële inputs 0 crashes; Owen/Romme/gezond gevalideerd. Zie STATUSRAPPORT_v1.9.4.md.
- **v3.8 (15 juni 2026):** na **grondige audit** (Bas+Claude) van engine v1.9.3. Twee bugs gevonden & gefixt: (1) KRITISCH — `volume_per_spier` rapporteerde het pre-plaatsings-plan i.p.v. het werkelijke schema (111 mismatches; prehab niet meegeteld + afrondingsverschil) → nu berekend uit het echte schema; (2) 1-set splinters bij krappe sessietijd (cap-check eiste som ≥4 i.p.v. ≥2 per dag) → SOP min-2-vloer nu correct gehandhaafd. Geaudit: 512-profielen-grid 0 crashes/0 invariant-schendingen, MEV-vloeren + 2x-frequentie gehaald bij ruime tijd, 13 adversariële inputs 0 crashes. Oordeel: productie-klaar voor schemageneratie. Zie STATUSRAPPORT_v1.9.3.md.
