...
ENGINE_VERSIE = "1.8.7-pplul-fill"
...
VROUW_CAP_BONUS = 1          # INTERIM_OPEN: bron "een tikje hoger", geen getal — geparkeerd
...
def verdeel_volume(weekbudget, voorkeur, kracht, flags, focus_accessoire=None,
    ...
    for spier in PRIMAIR:
        if spier in ACCESSOIRE_ROTATIE and spier != voorkeur:
            continue  # Billen: volume via accessoire-rotatie (tenzij voorkeurspier)
    ...
    if voorkeur in PRIMAIR:
        if voorkeur in ACCESSOIRE_ROTATIE:
    ...
    # Houding-focus: +2 directe schoudersets (achterkant), maar alleen als het
...
DAG_SPIEREN = {
    "FULL": PRIMAIR + ["Kuiten", "Core", "Onderrug"],
...
def verdeel_over_dagen(volume, dagen_types, dag_cap=None):
    ...
    for spier, sets_week in volgorde:
        ...
        gekozen = kand[:2]
        # cap-check: past 2+2 niet, val terug op 1 dag
        if dag_cap and (max(0, ruimte(gekozen[0])) + max(0, ruimte(gekozen[1]))) < 4:
...
def harde_filters(bib, spier, intake, effectieve_ervaring, cyclus=None):
    ...
    import re as _re
    blessure_tokens = set()
    for b in (intake.get("blessures") or []):
        blessure_tokens |= {t for t in _re.findall(r"[a-zà-ÿ]+", _lc(b)) if len(t) >= 3}
    pool = []
    for oef in bib:
...
def structuur_filter(pool, spier, dag_type, week_patronen):
    ...
    if spier == "Schouders":
        # Spillover-regio (SOP par.8): schouder-volume = lateraal/achter. GEEN pers
        # EN GEEN front raise — de voorste delt krijgt al volop pers-spillover.
        sub = [o for o in pool
...
def orden_sessie(items, voorkeur, cyclus=1, dag_type="FULL", flags=None):
    ...
    for it in items:
        ...
        if it.get("prehab") or sp in ("Kuiten", "Onderrug"):
...
def genereer(bib, intake, cyclus):
    ...
    FILL_POOLS = {
        "LOWER": [("Billen", 3), ("Kuiten", 2), ("Onderrug", 2)],
        "LEGS":  [("Billen", 3), ("Kuiten", 2), ("Onderrug", 2)],
        "UPPER": [("Schouders", 2), ("Core", 2)],     # rear-delt/houding + core
        "PUSH":  [("Schouders", 2), ("Core", 2)],
        "PULL":  [("Core", 2), ("Onderrug", 2)],
        "FULL":  [("Billen", 2), ("Core", 2)],
    }
    FILL_WEEK_CAP = {"Billen": 8, "Kuiten": 6, "Onderrug": 4, "Schouders": 4, "Core": 6}
    ...
    for d, (dag_naam, dag_type) in enumerate(template):
        ...
        for it in geordend:
            ...
            regels.append({"oefening": o.get("naam"), "spier": o.get("primaire_spier"),
    ...
    return {
        ...
        "cyclus_focus": {"focus": focus, "spier": focus_accessoire,
                         "reden": rationale},
        "volume_per_spier": {sp: sum(d.get(sp, 0) for d in per_dag)
                             for sp in alle_volume
                             if sum(d.get(sp, 0) for d in per_dag) > 0},
